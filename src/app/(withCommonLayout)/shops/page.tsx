...
const ShopPage = () => {
  ...
  const query: Record<string, any> = {
    ...
    brandId: selectedBrands.length ? selectedBrands[0] : undefined, // Assuming single brand for simplicity
    categoryId: selectedCategories.length ? selectedCategories[0] : undefined, // Assuming single category for simplicity
    priceRange: priceRange.join(','),  // Include other filters as needed
  };
console.log(query)
  const { data, isLoading, error,refetch } = useProductsQuery({ ...query });
  const {data:brandData, isLoading:brandLoading, error:brandError} = useBrandsQuery({});
  ...
  return (
      ...
      <Grid item xs={12} md={3} component={motion.div} initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.5 }}>
        <Box sx={{ p: 2, border: "1px solid #ccc", borderRadius: 2, backgroundColor: "#f4f4f4" }}>
                ...
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      ...
                      onChange={() => toggleSelection(category.id, setSelectedCategories, selectedCategories)}
                    />
                  }
          ...
          <Box sx={{ mb: 3 }}>
            ...
            {brandData?.data?.map((brand:IBrand, index:number) => (
              <FormControlLabel
                key={index}
                control={
                  <Checkbox
                    checked={selectedBrands.includes(brand.id)}
                    onChange={() => toggleSelection(brand.id, setSelectedBrands, selectedBrands)}
                  />
                }
                label={brand.title}
              />
    ...
    refetch(); // Assuming `refetch` is available from `useProductsQuery`
  }}>
            Apply Filters
          </Button>
        </Box>
